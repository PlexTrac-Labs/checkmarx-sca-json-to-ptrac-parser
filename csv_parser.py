...
class CSVParser():
    ...
    csv_headers_mapping_template = {
        ...
        "ExploitableMethods": {
            "header": "ExploitableMethods",
            "mapping_key": "no_mapping",
            "col_index": None
        },
        "Cwe": {
            "header": "Cwe",
            "mapping_key": "finding_cwe",
            "col_index": None
        },
        "IsViolatingPolicy": {
            "header": "IsViolatingPolicy",
            "mapping_key": "no_mapping",
            "col_index": None
        },
        "IsNewInRiskReport": {
            "header": "IsNewInRiskReport",
            "mapping_key": "no_mapping",
            "col_index": None
        },
        "Type": {
    ...
    def __init__(self):
        ...
        self.checkmarx_sca_file_prefix = f'checkmarx_sca_export'
    ...
    def handle_client(self, row):
        ...
        if header == None:
            ...
            value = row[index] # there could be an index problem if client_name is NOT used as a mapping_key in self.csv_headers_mapping_template - currently handled elsewhere
    ...
    def handle_report(self, row, client_sid):
        ...
        if header == None:
            ...
            index = self.get_index_from_header(header)
            value = row[index] # there could be an index problem if report_name is NOT used as a mapping_key in self.csv_headers_mapping_template - currently handled elsewhere
    ...
    def handle_finding(self, row, client_sid, report_sid):
        ...
        index = self.get_index_from_header(header)
        value = row[index] # there is checking in the parse_data func to prevent index errors here
    ...
    def handle_multi_asset(self, row, client_sid, finding_sid):
        ...
        index = self.get_index_from_header(header)
        value = row[index] # there could be an index problem if we DO use the asset_multi_name as a mapping_key in self.csv_headers_mapping_template
        if value == "":
            return
    ...
    def add_data_to_object(self, obj, obj_type, row):
        ...
        for value in self.csv_headers_mapping.values():
            ...
            if index == None: # CSV being processed doesn't have this column from the mapping, so the index never got set
                continue
            header = value['header']
